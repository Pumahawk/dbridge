...
public class SqlQueryExecutor {
  ...
  public Map<String, ? extends Object> query(QuerySpec spec, Map<String, Object> requestParams) {
    ...
    for (Query q : spec.getQueries()) {
      ...
      Object resultQuery =
          executeQuery(q.isUpdate(), q.getDatabase(), sql, namedSupporter.getStore());
  ...
  private Object executeQuery(
      boolean update, String database, String sql, Map<String, Object> properties) {
    NamedParameterJdbcTemplate jdbc = getJdbc(database);
    return update ? jdbc.update(sql, properties) : jdbc.queryForList(sql, properties);
  }

  private Object solveExpression(String expression, ScriptManager scriptManager) {
    return expression != null ? scriptManager.evaluate(expression) : null;
  }
  ...
  private NamedParameterJdbcTemplate getJdbc(String database) {
    Optional<? extends DataSource> opt =
        Optional.ofNullable(database).flatMap(databaseConnector::getById);
    ...
    DataSource ds =
        opt.isPresent()
            ...
            : databaseConnector
                .getDefault()
                .orElseThrow(() -> new RuntimeException("Unable to get find datasource."));
