...
public class ConfigurationLoader {
    ...
    public ConfigurationLoader(Environment env, ResourceLoader resourceLoader) {
        this.configurationDirectory = Optional.ofNullable(env.getProperty(Properties.CONFIGURATION_PATH))
            .map(resourceLoader::getResource)
            .map(r -> {
                try {
                    return r.getFile();
                } catch (IOException e) {
                    throw new ConfigurationException("Unable to get file from resource. Resource path: " + r.getFilename(), e);
                }
            })
            .filter(File::isDirectory)
            .orElseThrow(() -> new ConfigurationException("Unable get retrieve configuration directory"));
    ...
    private ConfigurationResource<? extends Object> configFromNode(JsonNode node) {
        return new ObjectMapper().convertValue(node, (Class<? extends ConfigurationResource<?>>) Optional
            .of(node)
            .map(n -> n.get("kind"))
            .map(JsonNode::asText)
            .map(Kind::fromName)
            .map(Kind::getType)
            .orElseThrow(() -> new ConfigurationException("Unable to retrieve kind from configuration node")));
    }
        ...
        for (File in : files) {
            if (in.isDirectory()) {
                ...
                try {
                    ...
                    throw new ConfigurationException("unable to load configuration file. name: " + in.getName(), e);
                }
            }
        }
        return list;
