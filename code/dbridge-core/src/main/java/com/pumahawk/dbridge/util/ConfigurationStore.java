...
public class ConfigurationStore {
  ...
  public Stream<Database> getDatabase() {
    return getDBridgeConfig()
        .map(DBridgeConfigResource::getSpec)
        .flatMap(s -> s.getDatabase().stream());
  }
  ...
  private void resolveExtension(List<Validator> vs) {
    List<GlobalValidator> globalValidators =
        getValidators()
  ...
  private void resolveExtension(
      List<Validator> vs, List<GlobalValidator> globalValidators, List<String> extended) {
