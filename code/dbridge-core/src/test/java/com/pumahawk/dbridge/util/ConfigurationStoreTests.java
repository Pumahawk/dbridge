...
public class ConfigurationStoreTests {
    ...
    public void addAndGetDBRidgeConfiguration() {
        ConfigurationStore cs = generateStoreWithElement(() -> new DBridgeConfigResource(), "name-dbridge-config");
    ...
    private ConfigurationStore loadStore() throws IOException {
        ...
        List<GlobalValidatorResource> gvs = yamlMapper.convertValue(config.get("global"), new TypeReference<List<GlobalValidatorResource>>(){});
        List<QueryResource> qs = yamlMapper.convertValue(config.get("queries"), new TypeReference<List<QueryResource>>(){});
        gvs.forEach(cs::add);
        qs.forEach(cs::add);
        return cs;
    }
    ...
    private ConfigurationStore generateStoreWithElement(Supplier<BasicResource<? extends Spec>> supplyResource, String name) {
        ConfigurationStore cs = new ConfigurationStore();
